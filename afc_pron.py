...
# 사용자로부터 서버 이름 입력 받기
user_input_1 = input("첫 번째 리전의 이름을 입력하세요[ex) seo]: ").lower()
user_input_2 = input("두 번째 리전의 이름을 입력하세요[ex) lgw]: ").lower()
...
# '2'와 '3'에 대한 비교 로직을 추가하세요.
    ...
    print("\nWAF 비교 결과:")
    print("첫 번째 서버 정보:")
    print("OLD_WAF_name:", OLD_region)
    print("OLD_WAF_public_ip:", OLD_info.get('W', {}).get('public_ip'))
    print("OLD_WAF_port:", OLD_info.get('W', {}).get('port'))
    print("OLD_WAF_user:", OLD_info.get('W', {}).get('user'))
    print("OLD_WAF_passwd:", OLD_info.get('W', {}).get('passwd'))
    ...
    if wp_sessid_cookie_old:
        ...
        # 중간 결과물 출력
        print("\n두 번째 서버 POST 요청 결과:")
        print(response_post_new.text)

        # Set-Cookie 값 추출
        wp_sessid_cookie_new = response_post_new.cookies.get("WP_SESSID")
        if wp_sessid_cookie_new:
            print("\n두 번째 서버 Set-Cookie 값:")
            print(wp_sessid_cookie_new)

            # 각 서버에 대한 GET 요청 보내기
            get_command_old = f"curl --insecure -b 'WP_SESSID={wp_sessid_cookie_old}' -H 'Content-Type:application/x-www-form-urlencoded' -s GET https://{OLD_info.get('W', {}).get('public_ip')}:443/webapi/conf/rule | jq 'map(select(.rule_name == \"Cross Site Scripting\" or .rule_name == \"File Upload\" or .rule_name == \"Response Header Filtering\" and .policy_name != null)) | .[] | select(.policy_name) | {{id: ._id, rule_name: .rule_name, level: .level, response_type: .response_type, policy_name: .policy_name}}'"
            ...
            get_command_new = f"curl --insecure -b 'WP_SESSID={wp_sessid_cookie_new}' -H 'Content-Type:application/x-www-form-urlencoded' -s GET https://{NEW_info.get('W', {}).get('public_ip')}:443/webapi/conf/rule | jq 'map(select(.rule_name == \"Cross Site Scripting\" or .rule_name == \"File Upload\" or .rule_name == \"Response Header Filtering\" and .policy_name != null)) | .[] | select(.policy_name) | {{id: ._id, rule_name: .rule_name, level: .level, response_type: .response_type, policy_name: .policy_name}}'"
            ...
            # 결과 출력
            print(f"\nWAF 비교 결과 for {OLD_info.get('W', {}).get('public_ip')}:")
            if get_response_old:
                print(get_response_old)
            else:
                print("첫 번째 서버 GET 요청 중 오류가 발생했습니다.")
            ...
            print("두 번째 서버에 대한 POST 요청 중 오류가 발생했습니다.")
    else:
        print("첫 번째 서버에 대한 POST 요청 중 오류가 발생했습니다.")
