...
with open('/home/common/cbric_git/latest/cbric/serverlist.json', 'r') as file:
    data = json.load(file)
...
if selected_instance == '1':
    ...
    for command in commands:
        print(f"\n--------------------'{command}' 비교 중--------------------")
        old_command_result = run_command(f"sshpass -p {OLD_info['A']['passwd']} ssh -o StrictHostKeyChecking=no -oHostKeyAlgorithms=+ssh-dss -p {OLD_info['P']['port']} {OLD_info['P']['user']}@{OLD_info['P']['public_ip']} {command}")
        new_command_result = run_command(f"sshpass -p {NEW_info['A']['passwd']} ssh -o StrictHostKeyChecking=no -oHostKeyAlgorithms=+ssh-dss -p {NEW_info['P']['port']} {NEW_info['P']['user']}@{NEW_info['P']['public_ip']} {command}")
