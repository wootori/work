...
def compare_internal_files(old_info, new_info, file_path):
    ...
    try:
        ...
        # 파일 내용 비교 (공백 무시)
        differ = difflib.ndiff(old_result.splitlines(), new_result.splitlines())
        diff_result_ignore_space = list(line for line in differ if line.startswith('- ') or line.startswith('+ '))
        print('\n'.join(diff_result_ignore_space))

        # 변경된 부분이 없으면 추가로 메시지 출력
        if not diff_result_ignore_space:
            print("변경된 부분이 없습니다. (공백 차이 무시)")
